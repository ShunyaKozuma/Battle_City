...
void Scene::startGame(){
    ...
    //std::copy(&background_num[0][0], &background_num[0][0] + HEIGHT * WIDTH, &Bullet.changedbackground_num);//先將未改變的背景代表值輸入進去
    //qDebug()<<Bullet.changedbackground_num;
    player = new Player();
    //player_health--;
    ...
    InvincibleDisplay = new QGraphicsTextItem();
    InvincibleDisplay->setFont(QFont("Invincible now",16));
    InvincibleDisplay->setDefaultTextColor(Qt::black);
    InvincibleDisplay ->setPos(655, 600);
    addItem(InvincibleDisplay);
    QString InvincibleString = QString("Invincible now");
    InvincibleDisplay->setPlainText(InvincibleString);
    InvincibleDisplay->hide();
    invinciable = new QTimer();
}
...
void Scene::keyPressEvent(QKeyEvent *event)
{
    ...
    if (event->key() == Qt::Key_R && CurrentStatus==2) {
        ...
        */
        ...
        //connect(bullet, &Bullet::hitbrick, this, &Scene::changemap);//test
...
void Scene::getPowerUps(){
    ...
    //PowerUp::PowerUpType randomType = static_cast< PowerUp::PowerUpType>(1);
            ...
            for (QGraphicsItem *item : items) {
                if (Enemy *enemy = dynamic_cast<Enemy*>(item)) {
                    ...
                    connect(invinciable, &QTimer::timeout,[=](){
                        ...
                        enemy->unsethelmet();
                        invinciable->stop();
                        //removeItem(InvincibleDisplay);
                        //delete InvincibleDisplay;
                        InvincibleDisplay->hide();
                    });
                    invinciable->start(10000);
                    InvincibleDisplay->show();
                    //無敵字幕顯示
...
void Scene::LosePLayerToScene()
{
    ...
    if(player_health==0)
    {
        ...
        //player = new Player();
        player->setPos(250, 250);
        player->setPixmap((QPixmap(":/images/player_up.png").scaled(40,40)));
        //addItem(player);
...
//void Scene::changemap(int x, int y)
//{
//    qDebug()<<(x/BLOCKUNIT);
//}
...
void Scene::saveAllRecord()//
{
    ...
    //saveRecordPath = "C:/Users/adm/Desktop/RecordForOldGame.txt";
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text))
    {
        ...
        //0-7儲存這些變數
        out << playerNumber << " " << gameLevel << " " << player->x() << " " << player->y() << " " << player_health << " " << totalplaytime << " "<<score->getScore()<<" ";
        out<<enemyLife<<" ";  //坦克數量、坦克的顯示器
        //8-9儲存是否有第二個玩家
        if (playerNumber == 2) {
    ...
    //tryyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy
    //saveRecordPath = "C:/Users/NUTC/OneDrive/桌面/RecordForOldGame.txt";
...
void Scene::getOldRecord()
{
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        ...
        { // 确保至少个值
                ...
                score->increase(values[6].toInt());

                //調整坦克的數量與生命值
                enemyLife=values[7].toInt();
                //tanknum=enemyLife;
                for(int i=enemyLife;i<tanknum;i++){
                    delete enemyHealthList.takeLast();}
                tanknum=enemyLife;
        ...
        {
                AddOnePlayer();
                player2->setX(values[8].toInt());
                player2->setY(values[9].toInt());
                Player2Exist=1;
        }
