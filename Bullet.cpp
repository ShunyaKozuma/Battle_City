...
//        delete item;
        ...
        //armor not have remain sheld --> into if state
        if(!armorflag){
            scene()->removeItem(item);
            delete item;
        }
    ...
    }else if(typeid(*item) == typeid(Player)&& dynamic_cast<Enemy_Bullet*>(this)->checkHelmet()){
        emit LosePlayerLife();
    }else if(typeid(*item) == typeid(Eagle))
    {
        emit GG();
    }
...
//                emit playerbulletgone();
            ...
            {
                emit playerbulletgone();
                qDebug() << "Billet Deleeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeet";
...
void Player_Bullet::freezeInPlace()
{
    //enemy_timer->stop();
    timer->stop();
...
void Player_Bullet::defreeze()
{
    timer->start();
}

void Enemy_Bullet::freezeInPlace(){
    enemy_timer->stop();
}

void Enemy_Bullet::defreeze(){
    enemy_timer->start();
}
...
void Player_Bullet::move()
{
    ...
    //if (x() + boundingRect().width() > 650 || y() < 0 || x() < 0 || y() + boundingRect().height() > 700){
    if (x() + boundingRect().width() > 650 || x() < 0){
        //widget???
        // Remove the bullet from the scene first
        //qDebug() << "DEleteeeeeeeeeeeeeeeeeeeeeee Bullet hit BOUDINGGGGGGGGGGGGGGGGGGGGGGGGGGGG";
        scene()->removeItem(this);
        delete this;
    }

/*
    QTimer* deletionTimer = new QTimer(this);
    connect(deletionTimer, &QTimer::timeout, [this]() {
        scene()->removeItem(this);
        delete this;
        qDebug() << "Enter Delete stronglyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy";
    });
    deletionTimer->start(10000);
*/
    //if(x() < -50 || y() < -50 || x() > )
...
Enemy_Bullet::Enemy_Bullet(int direction,int tank,bool helmets):dir(direction),tank_type(tank),helmet(helmets)
{
    ...
    connect(enemy_timer, &QTimer::timeout, this, &Bullet::move);
    enemy_timer->start(100);
    setZValue(0);
}
...
void Enemy_Bullet::move()
{
    ...
    if (x() + boundingRect().width() > 650 || y() < 0 || x() < 0 || y() + boundingRect().height() > 700) {
        // Remove the bullet from the scene first
        //qDebug() << "DEleteeeeeeeeeeeeeeeeeeeeeee Eneeeeeeeeeeeemyyyyyyyyyy Bullte";
        scene()->removeItem(this);
        delete this;
    }
